...
import type { ProjectDetail, PanoramaSettings } from "@/lib/types";
import {
  Upload,
  Loader2,
  Sparkles,
...
const SKY_MODE_OPTIONS = [
  { value: "skip", label: "Remove sky" },
  { value: "low_opacity", label: "Keep transparent" },
  { value: "keep", label: "Keep full" },
];

const DEPTH_MODE_OPTIONS = [
  { value: "single", label: "Single pass" },
  { value: "cubemap", label: "Cube faces (higher quality)" },
];
...
export default function PanoramaPage() {
  ...
  useEffect(() => {
    api.getProject(id).then((p: ProjectDetail) => {
      ...
      if (p.step === "training_complete" && p.has_output) {
        setStage("complete");
      } else if (p.step === "panorama_processing") {
        setStage("processing");
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const url = api.getPanoramaDepthPreview(id);
      const img = new Image();
      img.onload = () => {
  ...
  const handleFileSelect = useCallback((selectedFile: File) => {
    ...
    if (!validTypes.includes(selectedFile.type) && ext !== "hdr") {
      setError("Please select a valid image file (JPG, PNG, WebP, or HDR)");
      return;
    }
  ...
  const handleGenerate = async () => {
    ...
    try {
      // Upload the panorama
      await api.uploadPanorama(id, file, (pct) => {
      ...
      await api.runPanorama(id, settings);
    } catch (err: any) {
      setError(err.message || "Failed to process panorama");
      setUploading(false);
      setUploadPercent(null);
    }
  };
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="text-center">
        ...
        <h2 className="text-2xl font-bold text-white mb-2">
          Turn a 360 panorama into 3D
        </h2>
        <p className="text-gray-400">
          Upload an equirectangular panoramic image and generate an immersive 3D
          Gaussian splat in seconds
        </p>
      </div>
        ...
        {previewUrl ? (
          ...
          <div className="py-16 px-8 text-center">
            ...
            <p className="text-gray-300 font-medium mb-1">
              Drag & drop your 360 panoramic image
            </p>
            <p className="text-gray-500 text-sm">
              or click to browse -- .jpg, .png, .webp, .hdr (equirectangular
              2:1 ratio)
            </p>
          </div>
        )}
          ...
          <div className="px-4 pb-4 space-y-4 border-t border-gray-700/50 pt-3">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex gap-2">
                {SKY_MODE_OPTIONS.map((opt) => (
                  <button
                    key={opt.value}
                    onClick={() =>
                      setSettings((s) => ({
                        ...s,
                        sky_mode: opt.value as PanoramaSettings["sky_mode"],
                      }))
                    }
                    className={`px-3 py-1.5 rounded-lg text-sm font-medium transition ${
                      settings.sky_mode === opt.value
            ...
            <p className="text-xs text-gray-500 ml-40">
              How to handle distant sky/background pixels.
            </p>

            {/* Depth mode */}
            <div className="flex items-center gap-4">
              <label className="text-sm text-gray-400 w-36">Depth Mode:</label>
              <div className="flex gap-2">
                {DEPTH_MODE_OPTIONS.map((opt) => (
                  <button
                    ...
                    onClick={() =>
                      setSettings((s) => ({
                        ...s,
                        depth_mode: opt.value as PanoramaSettings["depth_mode"],
                      }))
                    }
                    className={`px-3 py-1.5 rounded-lg text-sm font-medium transition ${
                      settings.depth_mode === opt.value
                        ? "bg-teal-600 text-white"
                        : "bg-gray-700 text-gray-400 hover:text-white"
                    }`}
                  >
            ...
            <p className="text-xs text-gray-500 ml-40">
              Single pass is fast. Cube faces gives better edge quality.
            </p>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
            ...
            <div className="relative">
              {previewUrl && (
                <img
                  src={previewUrl}
                  alt="Panorama"
                  className={`w-full object-contain max-h-[300px] transition-opacity duration-500 ${
          ...
          {depthPreviewUrl && depthLoaded && (
            <div className="flex items-center gap-2 text-xs text-teal-400 animate-fade-in">
          ...
          <div className="bg-gray-800/50 rounded-xl border border-gray-700/50 p-6">
            ...
            <div className="space-y-2">
              {PROCESSING_STEPS.map((step, i) => {
                const stepKey = step.toLowerCase().replace("...", "").trim();
                const isActive = processingStep
                  .toLowerCase()
                  .includes(stepKey);
                ...
                const currentIdx = PROCESSING_STEPS.findIndex((s) =>
                  processingStep
                    .toLowerCase()
                    .includes(s.toLowerCase().replace("...", "").trim())
                );
    ...
    <div className="space-y-6">
      ...
      <div className="text-center py-4">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-green-500/10 border border-green-500/20 text-green-400 mb-3">
          ...
          <span className="font-medium">Panorama 3D model ready</span>
        </div>
        {gaussianCount && (
      ...
      <div className="flex flex-wrap items-center justify-center gap-3">
        <button
          ...
          className="bg-teal-600 hover:bg-teal-700 text-white px-5 py-2.5 rounded-lg flex items-center gap-2 transition font-medium"
        >
  ...
  const renderError = () => (
    <div className="max-w-lg mx-auto text-center py-12">
      ...
      <button
        onClick={handleReset}
        className="bg-teal-600 hover:bg-teal-700 text-white px-6 py-2.5 rounded-lg flex items-center gap-2 transition mx-auto"
      >
        <RotateCcw className="w-4 h-4" />
        Try Again
      </button>
